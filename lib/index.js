...
FileTransformCache.prototype._fileFromCacheEntry = function(origFile, cacheEntry) {
    var cachedFile = new File({path:     cacheEntry.path,
                               base:     origFile.base,
                               contents: new Buffer(cacheEntry.content)});
    ...
    if ( cacheEntry.destPath !== cacheEntry.path ) {
        cachedFile.path = cacheEntry.destPath; /* do this way to perserve history */
    }
    return cachedFile;
};
...
FileTransformCache.prototype._getFileHash = function(file) {
    ...
    return crypto.createHash('sha1').update(fileContent||'emptyfile').digest('hex');
}
