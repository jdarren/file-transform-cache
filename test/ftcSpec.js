...
describe('file-transform-cache', function() {
    ...
    describe('caching', function () {
        ...
        it('should run the transform when cache not present, but not otherwise', function (done) {
            ...
            var ftc = ftcache({path: path.join( __dirname, '.sample'), transforms: [bracketTransform(transformOpts)]});
        ...
        it('should run the transform when the cache is stale', function (done) {
            ...
            var ftc = ftcache({path: path.join( __dirname, '.sample2'), transforms: [bracketTransform(transformOpts)]});
            ftc.transform( helloFile, (err, resultFile) => {
                ...
                setTimeout( () => {
                    ...
                    ftc.transform( helloFile, (err, resultFile) => {
                        ...
                        assert.equal('[howdy]', ftc._cache[helloFile.path].content ); // cache is updated with the new value
    ...
    describe('saving', function () {
        ...
        it('save() should save when the cache is dirty', function (done) {
            ...
            var ftc = ftcache({path: cacheFileName, transforms: [bracketTransform(transformOpts)]});
            ftc.transform( helloFile, (err, resultFile) => {
                ...
                try {
                    ...
                    var cacheFile = JSON.parse(fs.readFileSync(cacheFileName, 'utf-8'));
                    assert.equal('[greetings]', cacheFile[helloFileName].content);
